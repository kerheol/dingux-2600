...
const char* DEFAULT_ROM_PATH = "rom_path";
...
int
loc_atari_save_paths(char* path_type, char path[])
{
  ...
  snprintf(FileName, MAX_PATH, "%s/set/default_paths.set", ATARI.atari_home_dir);
...
loc_atari_load_paths()
{
   ...
   snprintf(file_name, MAX_PATH, "%s/set/default_paths.set", ATARI.atari_home_dir);
   fp = fopen(file_name, "r");
   if (fp == (FILE *)0 ) return 0;
   line = (char *)malloc(MAX_PATH*2);

   while ((read = getline(&line, (size_t*)&len, fp)) != -1) {
      options = str_split(line, '=');
      if(options){
         if(strcmp(options[0], DEFAULT_ROM_PATH) == 0){
            strcpy(ATARI.atari_rom_path, options[1]);
         }
      }
   }
...
atari_load_rom(char *FileName, int zip_format)
{
  ...
  if (! error ) {
    ...
    loc_atari_save_paths((char *)DEFAULT_ROM_PATH, file_path);
  }
...
psp_global_initialize()
{
  ...
  strcpy(ATARI.atari_home_dir,".");
   ...
   home_name = (char*)malloc(sizeof(char)*6 + 1);
   strcpy(home_name,".atari-2600");

   sprintf(ATARI.atari_home_dir, "%s/%s/", getenv("HOME"), home_name);
   mkdir(ATARI.atari_home_dir, 0777);

   tmp_directory = (char*)malloc(MAX_PATH + 1);
   ...
   strcpy(ATARI.atari_rom_path, tmp_directory);
   mkdir(tmp_directory, 0777);
   free (tmp_directory);

   tmp_directory = (char*)malloc(MAX_PATH + 1);
   sprintf(tmp_directory, "%s/%s/state/", getenv("HOME"), home_name);
   mkdir(tmp_directory, 0777);
   free (tmp_directory);
